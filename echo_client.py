...
send_full_message(client_socket, message.encode(), end_of_message, destination_address, buff_size_server, message_loss_percentage=desired_message_loss)

print("Se ha hecho enviado el mensaje del mensaje:\n {}".format(message))
...
print("Se ha recibido desde servidor echo: {}".format(received_message.decode()))

# Si los mensajes son distintos entonces hubo pérdida
if received_message.decode() != message:
    print("--------> Hubo pérdida, los mensajes no son iguales!")
else:
    print("--------> los mensajes son iguales :D")